...
const StreetHub: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen pb-20 space-y-16 -mt-6 -mx-4 md:-mx-0 transition-colors duration-1000 ${isUnfiltered ? 'grayscale' : theme === 'dark' ? 'bg-[#050505]' : 'bg-gray-50'}`}>
      ...
      <section className="relative h-[550px] md:h-[700px] overflow-hidden bg-black flex items-center justify-center">
         ...
         <div className="relative z-10 flex flex-col items-center text-center px-6">
            ...
            <h1 className="font-embroidery text-6xl md:text-9xl text-white italic leading-none mb-6">
      ...
      <div className="max-w-7xl mx-auto px-6">
         <div className={`${theme === 'dark' ? 'bg-[#111] border-white/5' : 'bg-white border-gray-100 shadow-xl'} p-6 rounded-[3rem] border transition-all flex flex-col lg:flex-row items-center justify-between gap-8 mb-16`}>
            <div className={`flex p-2 rounded-[2rem] border ${theme === 'dark' ? 'bg-black border-white/5' : 'bg-gray-50 border-gray-200'}`}>
               <TabBtn label="Live Street Wire" active={activeTab==='wire'} onClick={()=>setActiveTab('wire')} theme={theme} />
               <TabBtn label="Challenges" active={activeTab==='challenges'} onClick={()=>setActiveTab('challenges')} theme={theme} />
               <TabBtn label="Heatmap" active={activeTab==='heatmap'} onClick={()=>setActiveTab('heatmap')} theme={theme} />
            </div>
            ...
            <div className="flex flex-1 items-center gap-6 w-full lg:w-auto">
               <div className="relative flex-1 group">
                  ...
                  <input 
                     ...
                     className={`w-full pl-12 pr-4 py-4 border rounded-2xl outline-none focus:border-red-600/50 transition-all text-xs font-bold uppercase tracking-widest ${theme === 'dark' ? 'bg-black border-white/5 text-white' : 'bg-white border-gray-200 text-gray-900'}`}
                     value={searchQuery}
         ...
         {activeTab === 'heatmap' && (
            <div className={`${theme === 'dark' ? 'bg-[#111] border-white/5' : 'bg-white border-gray-200 shadow-xl'} relative h-[600px] rounded-[4rem] border overflow-hidden flex items-center justify-center transition-all`}>
               ...
               <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-12">
                  ...
                  <h3 className={`font-embroidery text-5xl italic mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>LIVE URBAN HOTSPOTS</h3>
                  ...
                  <div className="mt-12 flex gap-10">
                     <Hotspot label="Luthuli Ave" count={14} theme={theme} />
                     <Hotspot label="Tom Mboya" count={22} theme={theme} />
                     <Hotspot label="Downtown Kampala" count={8} theme={theme} />
                  </div>
               </div>
            </div>
         )}
         ...
         {selectedPhoto && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-xl overflow-y-auto">
               <motion.div 
                  ...
                  className={`${theme === 'dark' ? 'bg-[#0a0a0a] border-white/5' : 'bg-white border-gray-100'} rounded-[4rem] shadow-2xl overflow-hidden max-w-6xl w-full flex flex-col lg:flex-row border`}
               >
                  ...
                  <div className="flex-1 p-12 flex flex-col">
                     <div className="flex justify-between items-start mb-10">
                        <div>
                           ...
                           <h2 className={`font-embroidery text-5xl italic leading-tight mb-2 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{selectedPhoto.title}</h2>
                           <p className="text-gray-500 text-xs font-black uppercase tracking-widest">Captured by <span className={`${theme === 'dark' ? 'text-white' : 'text-gray-900'} underline`}>{selectedPhoto.photographer}</span></p>
                        </div>
                     ...
                     <div className="space-y-8 flex-1">
                        <div className={`p-6 rounded-3xl border ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-gray-50 border-gray-200'}`}>
                           ...
                           <div className="grid grid-cols-2 gap-6">
                              <div>
                                 ...
                                 <span className={`text-xs font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{selectedPhoto.gear?.camera || 'N/A'}</span>
                              </div>
                              <div>
                                 <span className="block text-[8px] font-black text-gray-500 uppercase mb-1">Optics</span>
                                 <span className={`text-xs font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{selectedPhoto.gear?.lens || 'N/A'}</span>
                              </div>
                           </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                           <div className="flex items-center gap-3">
                              ...
                              <div>
                                 ...
                                 <span className={`text-sm font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{selectedPhoto.decisiveMomentSeconds}s After Arrival</span>
                              </div>
                           </div>
                        </div>
                     </div>
                     ...
                     <div className="mt-12 space-y-4">
                        ...
                        <button className={`w-full font-black py-5 rounded-[2rem] transition-all text-[10px] uppercase tracking-widest border ${theme === 'dark' ? 'bg-white/5 border-white/10 text-white hover:bg-white hover:text-black' : 'bg-gray-100 border-gray-200 text-gray-900 hover:bg-gray-200'}`}>
...
const StreetPhotoCard: React.FC<{photo: Photo, onDetails: () => void, onBuy: () => void, theme: string}> = ({ photo, onDetails, onBuy, theme }) => (
  <motion.div 
    ...
    whileHover="hover"
    onClick={onDetails}
    className={`group relative rounded-[2.5rem] overflow-hidden border transition-all cursor-pointer shadow-xl ${theme === 'dark' ? 'bg-[#111] border-white/5' : 'bg-white border-gray-100'}`}
  >
      <div className="relative">
         ...
         <div className="absolute inset-x-0 bottom-0 p-8 translate-y-full group-hover:translate-y-0 transition-transform duration-500 bg-black/40 backdrop-blur-md border-t border-white/10 z-30">
            <div className="flex justify-between items-center text-white">
               ...
               <button onClick={(e) => { e.stopPropagation(); onBuy(); }} className="bg-red-600 p-3 rounded-2xl shadow-xl hover:bg-white hover:text-red-600 transition-all"><ShoppingCart size={18} /></button>
            </div>
         </div>
      </div>
  </motion.div>
...
const ChallengeCard = ({ title, desc, prize, ends, image, theme }: any) => (
  <div className={`p-10 rounded-[3rem] border hover:border-red-600/30 transition-all group overflow-hidden relative ${theme === 'dark' ? 'bg-[#111] border-white/5' : 'bg-white border-gray-100 shadow-xl'}`}>
    ...
    <div className="relative z-10">
      ...
      <h3 className={`text-3xl font-black uppercase tracking-tighter mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{title}</h3>
      <p className="text-gray-500 text-sm leading-relaxed mb-8">{desc}</p>
      <div className={`p-4 rounded-2xl ${theme === 'dark' ? 'bg-red-600/10 border border-red-600/20' : 'bg-red-50 border border-red-100'}`}>
        <span className="text-[10px] font-black text-red-600 uppercase block mb-1">Prize Pool</span>
        <span className={`text-xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{prize}</span>
      </div>
    </div>
  </div>
);

const Hotspot = ({ label, count, theme }: any) => (
  <div className="flex flex-col items-center">
    ...
    <span className={`font-bold text-xs ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{label}</span>
